...
/* Observa que sienpre es nesesario declarar el numero de columnas.Si no lo haces, el compilador marcara un error de sintaxis. */
...
/* LA funcion Lectura se utiliza para leer un arreglo bidimencional. Observa que solo se debe pasar como parametro el numero de filas ya que la matriz es cuadrada. */
{
    ...
    for (I=0; I<F; I++)
        for (J=0; J<F; J++)
    {
        printf("Ingrese ele elemento %d %d: ", I+1, J+1);
        scanf("%d", &A[I][J]);
    }
}
