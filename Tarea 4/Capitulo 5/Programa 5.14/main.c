...
/* Estadistico.
El programa,al recibir como dato un arreglo unidimencional de enteros que contiene calificaciones, calcula la madia, la varianza,
la desviacion estandar y la moda. */

const int max = 100;

void Lectrura (int*, int);
float Media(int *, int);
float Varianza(int *,int, float);       /* Prototipo de funciones. */
...
/* La funcion Lectrura se utiliza para leer un arreglo unidimencional de T elementos de tipo entero. */
{
    int I;
    for (I=0; I<T; I++)
    {
